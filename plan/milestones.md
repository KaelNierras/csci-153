...
| **2** | **The Contract** | W4–6 | W4–6 lab | 67 | `openapi.yaml` with a data model behind it, types generated, screens on the mock |
...
The five old decks were re-cut into these four. Nothing was discarded except the old Module
2's orientation and closing slides, which belonged to a module that no longer exists.
...
| 3.3 | **2.5** | Arrays, objects, iteration |
| 3.4 | **2.6** | Functions, modules, file layout |
| 3.5 | **2.7** | JSON, `fetch`, and `await` |
| 3.6 | **2.8** | What React does for you |
| 3.7 | **2.9** | Choosing a library |
| 3.8 | **2.10** | Does it work? — Vitest |
| 4.2 | **3.1** | Migrations |
...
3. **The whole old Module 3 became the back half of Module 2.** Two of its three anchor
   files — `schema.d.ts` and `client.ts` — are produced by the contract.
...
| **CO3** | 3.1–3.7 | W5–6, lessons 2.3–2.9 | A5, A6 (W6) | none |
...
| **Module 2 is 67 slides across three weeks** and carries two COs. It is the heaviest deck. | It is also the only three-week module. The tour half (2.3–2.10) is short sessions with reading done in lab, which is how the old Module 3 already worked. |
