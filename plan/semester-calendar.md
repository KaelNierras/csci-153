...
| **5** | 2 | Contract authoring, continued — the four worked examples and the spec explorer | **Prior-knowledge diagnostic** · generate the types; screens on the Prism mock | **A4** The contract *(group)* |
...
demonstration apiece and no exercises. The demonstrations are already chosen: the test
ceiling broken on purpose (2.3), the anon key reading another user's row (3.3), and the
`VITE_` secret found in the bundle (3.5).
...
| A5 Screens on your mock *(group)* | 3% | W6 | 2 — **gate** |
| A6 First unit tests | 3% | W6 | 2 |
